...
export default function OrdersPage() {
  ...
  const handleCancel = async (orderId) => {
    try {
      ...
      addToast(t('orders.orderCancelled', 'Order cancelled and stock restored.'), 'success');
    } catch (apiError) {
      addToast(getErrorMessage(apiError), 'danger');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC]">
      <div className="max-w-[1440px] mx-auto px-8 py-10">
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-[#64748B] mt-1">{t('orders.subtitle', 'Review your previous purchases, track status, and cancel eligible orders.')}</p>
          </div>
          <button
        ...
        {loading ? (
          ...
          <Card className="border border-dashed border-[#BFDBFE] bg-white p-10 text-center">
            ...
            <h2 className="mt-4 text-lg font-semibold text-[#1E293B]">{t('orders.noOrders', 'No orders yet')}</h2>
            <p className="mt-2 text-sm text-[#64748B]">{t('orders.noOrdersDesc', 'Your recent purchases will appear here once you place an order.')}</p>
            <Link
              to="/shop"
            ...
            >
              {t('orders.browseShop', 'Browse the shop')}
            </Link>
          </Card>
        ) : (
          <div className="space-y-4">
                    ...
                    <div className="min-w-0 flex-1 text-left">
                      <div className="space-y-2">
                        <div className="flex flex-wrap items-center gap-2">
                          <h2 className="text-base font-semibold text-[#1E293B]">{t('orders.order', 'Order')} {order.orderNumber || order._id}</h2>
                        ...
                        <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm text-[#64748B]">
                          ...
                          <span>{order.items?.length ?? 0} {t('shop.items', 'item(s)')}</span>
                          <span>{formatCurrency(order.total ?? order.totalAmount ?? 0)}</span>
                        </div>
                        <p className="text-sm text-[#64748B]">
                          {t('orders.deliveryFee', 'Delivery fee')} {formatCurrency(order.deliveryFee ?? 0)} • {t('orders.payment', 'Payment')} {String(order.paymentMethod || 'cod').toUpperCase()}
                        </p>
                        <div className="space-y-1 mt-3 text-sm text-[#475569]">
                          ...
                          {order.items?.length > 2 ? (
                            <p className="truncate">{t('orders.andMore', 'and {{count}} more items', { count: order.items.length - 2 })}</p>
                          ) : null}
                        </div>
                      </div>
                    </div>
      ...
      <Modal
        ...
        size="lg"
        title={selectedOrder ? `${t('orders.order', 'Order')} ${selectedOrder.orderNumber || selectedOrder._id}` : t('orders.orderDetails', 'Order details')}
      >
        {selectedOrder ? (
          <div className="px-6 pb-6 space-y-6">
            <div className="grid gap-4 md:grid-cols-2">
              <div className="rounded-2xl border border-[#E2E8F0] bg-[#FAFBFD] p-4 space-y-2 text-sm text-[#475569]">
                <p><span className="font-semibold text-[#1E293B]">{t('orders.status', 'Status')}:</span> {selectedOrder.status || 'placed'}</p>
                <p><span className="font-semibold text-[#1E293B]">{t('orders.payment', 'Payment')}:</span> {String(selectedOrder.paymentMethod || 'cod').toUpperCase()}</p>
                <p><span className="font-semibold text-[#1E293B]">{t('orders.created', 'Created')}:</span> {formatDate(selectedOrder.createdAt)}</p>
                <p><span className="font-semibold text-[#1E293B]">{t('orders.updated', 'Updated')}:</span> {formatDate(selectedOrder.updatedAt)}</p>
                <p><span className="font-semibold text-[#1E293B]">{t('orders.total', 'Total')}:</span> {formatCurrency(selectedOrder.total ?? selectedOrder.totalAmount ?? 0)}</p>
              </div>
              {selectedOrder.deliveryAddress ? (
                <div className="rounded-2xl border border-[#E2E8F0] bg-[#FAFBFD] p-4 text-sm text-[#475569] space-y-1">
                  <p className="font-semibold text-[#1E293B]">{t('orders.deliveryAddress', 'Delivery address')}</p>
            ...
            <div className="space-y-3">
              <h4 className="text-sm font-semibold uppercase tracking-wide text-[#64748B]">{t('orders.items', 'Items')}</h4>
              <div className="space-y-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            ...
                            <p className="text-sm text-[#64748B]">{t('orders.qty', 'Qty')} {item.quantity}</p>
                            {seller.name ? <p className="text-xs text-[#64748B] mt-1">{t('orders.seller', 'Seller')}: {seller.name}{seller.phone ? ` · ${seller.phone}` : ''}</p> : null}
                          </div>
                          <p className="text-sm font-semibold text-[#0046CE]">{formatCurrency(item.price)}</p>
                        </div>
                        <p className="mt-2 text-sm text-[#64748B]">{t('orders.lineTotal', 'Line total')} {formatCurrency((item.price || 0) * (item.quantity || 0))}</p>
                      </div>
                    </div>
                  );
                })}
