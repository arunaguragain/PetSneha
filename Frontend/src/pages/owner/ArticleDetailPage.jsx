...
export default function ArticleDetailPage() {
  ...
  if (!article) {
    return <Card className="p-8">{t('articles.noneFound', 'Article not found.')}</Card>;
  }
    ...
    <div className="max-w-[1440px] mx-auto px-8 py-10">
      <Card className="p-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2"><p className="font-semibold text-neutral-900 text-lg">{article.author?.name || article.authorName || t('forum.member', 'Vet author')}</p><VerifiedBadge /></div>
            <p className="text-body-md text-neutral-500">{formatDate(article.publishedAt || article.createdAt)}</p>
          </div>
        </div>
        <Badge className="mt-4" variant="primary">{t(`articles.${(article.category || 'general').toLowerCase()}`, article.category || 'General')}</Badge>
